...
@Client.on_message(filters.command('show_caption'))
async def show_caption(bot, message):
    user_id = message.from_user.id
    caption = await db.get_caption(user_id)
    if not caption:
       return awair message.reply_text("You didn't added any custom caption")
    await message.reply_text(f"<b>Your Caption:\n\n`{caption}`")
